...
async fn ollama_no_auth_sends_no_authorization_header() {
    ...
    assert!(!http.has_header("authorization"));
    assert!(!http.has_header("x-api-key"));
}
...
async fn bearer_and_custom_header_credentials() {
    ...
    provider
        ...
        .await
        .unwrap();
    assert!(mock.requests()[0].headers.iter().any(|(name, value)| {
    ...
    let provider = provider_with(
        &mock,
        ProviderConfig::openai_chat(Credentials::header("x-litellm-api-key", "k1"))
            .with_base_url(Url::parse("http://proxy.internal/v1").unwrap()),
    );
    provider
        .language_model("m")
