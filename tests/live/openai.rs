...
async fn openai_chat_agentic_loop() {
    ...
    let model = provider.language_model("gpt-5.6-luna");
    // gpt-5.x rejects function tools on Chat Completions unless a reasoning
    // effort is set explicitly. See ProviderConfig::openai_chat.
    let mut request = loop_request();
...
async fn compaction_rejected_on_chat_protocol() {
    ...
    let error = provider
        ...
        .await
        .unwrap_err();
    assert_eq!(error.kind(), cogfer::ErrorKind::UnsupportedCapability);
}
...
async fn openai_sub_minimum_compaction_threshold_is_clamped() {
    ...
    let request = Request::builder()
        ...
        .compaction(cogfer::Compaction {
            trigger_input_tokens: Some(500),
            ..cogfer::Compaction::default()
        })
